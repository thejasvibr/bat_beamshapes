...
from sympy import  Piecewise, IndexedBase
...
one_n_S_mr = numerator_n_S_mr/denominator_n_S_mr

# eg. m=2, n=1, r=0 --> use solveset(denomterm1,m) to get the pole points
# at r = 2m +1 
# at r = 2m - 2n -2 
denomterm2 = r*0.5+n-m+1
denomterm1 = 0.5*r-m-0.5
...
ssum_f = lambdify([m,n,k,a,NN], S_sumterm,'sympy')
...
nbmka_f = lambdify([m,n,k,a,NN], n_B_mka, 'sympy')
...
one_Mmn_term = lambdify([a,b,k,m,n,NN], M_mn)

#%% 
# eqn. 13.227

b_term = - Sum((conjugate(m_B_p_kb)/(p+1))*(a/b)**(2*p),  (p,0,P))
bterm_func = lambdify([a,b,k,m,NN], b_term)
...
if __name__ == '__main__':
    ...
    #%% 
    import numpy as np 
    angles = [pi*each for each in [0,1/12, 1/6, 1/4, 1/3, 1/2,2/3, 1]]
    angles_rad = [each.evalf() for each in angles]
    
    at_theta = []    
    for angle in angles:
        at_theta.append( d_thetaf(k_v,a_v,b_v,angle,Ann,N_max).evalf())
    db_theta = 20*np.log10(np.array(np.abs(at_theta),'float64'))
